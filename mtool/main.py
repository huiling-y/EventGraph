...
def read_graphs(stream, format = None,
  ...
  if format in {"amr", "camr"}:
    ...
    generator = codec.ace.read(stream, text=text)
  elif format == "pmb":
...
def main():
  ...
  if arguments.write is not None and \
     ...
     {"dot", "tikz", "displacy", "evaluation", "id", "json", "mrp",
      "source", "targets", "txt", "ucca", "norec", "ace"}:
  ...
  if arguments.write == "ace":
    ace_graphs = [codec.ace.write(graph, graph.input) for graph in graphs]
    json.dump(ace_graphs, arguments.output, indent=None)
