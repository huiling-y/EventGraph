# based on https://github.com/thunlp/HMEAE/blob/master/utils.py 
...
class Extractor():
    ...
    def find_index(self, offsets, offset):
        idx_start = -1
        idx_end = -1
    ...
    def Files_Extract(self):
        ...
        for dir in self.dirs:
            path = self.ace_path + '/' + dir + '/timex2norm'
            ...
            for file in files:
                ...
                for mention_tag in mention_tags:
                    ...
                    for sample in mention:
                        # inclusive charactor spans, in original document
    ...
    def Event_Extract(self):
        nlp = StanfordCoreNLPv2(self.corenlp_path)
        offsets2idx = {}
        for dir in self.dirs:
            path = self.ace_path + '/' + dir + '/timex2norm'
            files = self.event_files[dir]
            for file in files:
                ...
                for event in events:
                    ...
                    for event_mention in event_mentions:
                        # all spans are inclusive charactor spans, in original document
                        event_info = event_mention.getElementsByTagName('ldc_scope')[0].getElementsByTagName('charseq')[0] # event mention
                        sent = str(event_info.childNodes[0].data)
                        start = int(event_info.getAttribute('START')) # event mention start idx
                        end = int(event_info.getAttribute('END')) # event mention end idx
                        ...
                        map_entity = {(e['start'], e['end']):i for i,e in enumerate(entities)} # entity span: idx in the entities list
                        ...
                        tokens, offsets = nlp.word_tokenize(sent, True) # event mention sentence tokenized
                        ...
                        trigger_s, trigger_e = self.find_index(find_offsets, (trigger_start, trigger_end)) # offset of trigger in sent
                        trigger_offsets = tokens_offsets[trigger_s:trigger_e+1]
                        trigger_tokens = tokens[trigger_s:trigger_e+1]
                        _entities = []
                        for e in entities:
                            idx_start, idx_end = self.find_index(find_offsets, (e['start'], e['end'])) # offset of argument in sent
    ...
    def None_event_Extract(self):
        ...
        for dir in self.dirs:
            ...
            for file in files:
                ...
                with open(path + '/' + file, 'r') as f:
                    text = f.read()
                sents, offsets = nlp.sent_tokenize(text) # offsets are the whole doc
                sents, offsets = self.correct_offsets(sents, offsets) # update offsets per sentence
                ...
                for i, sent in enumerate(sents):
                    ...
                    tokens_offsets = [(e[0], e[1]-1) for e in offset]
                    event_type = 'None'
                    trigger_tokens = []
                    trigger_offsets = []
                    trigger_start = -1
                    ...
                    for e in _entities:
                        ...
                        entity_info = {
                            'tokens': sent[idx_start:idx_end+1],
                            'role': 'None',
    ...
    def process(self):
        ...
        for none_event in self.None_events:
            _none_event = {
                ...
                'event_type': 'None',
                'trigger_tokens': [none_event['tokens'][0]],
                'trigger_start': 0,
                'trigger_end': 0,
                'trigger_offsets': [none_event['offsets'][0]],
